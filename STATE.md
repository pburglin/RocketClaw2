...
- `harness-plan`, `harness-run`, `harness-run-plan`, `auto-code`, and `llm-query` now stream LLM text by default with `--no-stream` opt-out; `--verbose` layers formatted request/response inspection on top.
