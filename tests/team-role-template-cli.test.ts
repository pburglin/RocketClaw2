...
describe('team-role-template CLI', () => {
  ...
  it('can derive a reviewer template from a saved handoff artifact', async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), 'rocketclaw2-team-role-home-'));
    const handoffDir = path.join(home, '.rocketclaw2', 'handoffs');
    await fs.mkdir(handoffDir, { recursive: true });
    await fs.writeFile(path.join(handoffDir, 'handoff-123.json'), JSON.stringify({
      id: 'handoff-123',
