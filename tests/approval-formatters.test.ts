...
describe('approval formatters', () => {
  it('formats approval lists and summaries for operators', () => {
    const items = [
      ...
      { id: '3', kind: 'harness-plan' as const, target: 'run-123', detail: 'Review plan', status: 'pending' as const, createdAt: 't3' },
    ];
    expect(formatApprovals(items)).toContain('file-management');
    expect(formatApprovalSummary(items)).toContain('Pending: 2');
  });
});
