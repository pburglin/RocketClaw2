...
- `auto-code` now resumes interrupted work for the same workspace/task from the latest incomplete saved run after Ctrl+C instead of always replanning from scratch; if no incomplete run exists, it can also reuse the latest approved matching plan.
